...
export const getRuta = async (req, res) => {
    try {
      const resp = await consul.query('SELECT ruta.* , usuario.nombre, usuario.preferenciasviaje, usuario.puntuacion,vehiculo.fotovehiculo FROM ruta, usuario, vehiculo where usuario.id=ruta.idusuarioconductor and usuario.id=vehiculo.idusuario and estado= true and ruta.id=$1', [req.params.id]);
